...
export function Accordion({ items, mode = "multi" }: AccordionProps) {
  ...
  return (
    <div className="divide-y divide-brand-mist rounded-2xl border border-brand-mist bg-white">
        ...
        return (
          <div key={item.question}>
            ...
            >
              ...
              <span
                aria-hidden
                className={`shrink-0 text-brand-muted transition-transform duration-150 ${isOpen ? "rotate-45" : ""}`}
              >
                +
              </span>
            </button>
            {isOpen && (
              <div id={panelId} role="region" aria-labelledby={buttonId} className="px-5 pb-4 text-sm text-brand-muted">
                {item.answer}
              </div>
            )}
          </div>
